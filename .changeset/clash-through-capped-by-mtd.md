...
The reported depth of a through-penetration is now capped by the exact box depth. That distance is proven to separate the pair, so a larger number over-reports it. Both sides of such a tie now report the same 26 mm, and the result is still labelled an estimate. On 11 sample models (about 370 clash records), no reported depth or label changes.
