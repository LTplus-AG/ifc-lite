...
`packages/codegen/generated/ifc4/type-ids.ts`, `packages/parser/src/generated/type-ids.ts`, and `rust/core/src/generated/schema.rs` were regenerated to correct the same two cells; each diff is exactly those two constants. `packages/codegen/generated/ifc4x3/type-ids.ts` embeds the same table and still carries the same two corrupted cells — it is not touched by this patch and remains wrong until it is regenerated separately.

`formatCRC32TableLiteral()` now validates `perLine` and throws for a zero, negative, or non-integer value instead of silently producing a broken or extremely slow result. No caller passes a non-default `perLine` today, so this is a hardening of the exported helper's contract rather than a behavioral fix to generated output — with the default (`perLine = 6`), this hardening by itself leaves the three regenerated artifacts above unchanged.
