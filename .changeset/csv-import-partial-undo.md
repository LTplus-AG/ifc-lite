...
- A run is recorded chunk by chunk under one batch id, so it is still one undo step, and an edit made while it yields keeps its place in history. Previously the run was recorded only at the end, on top of that edit, and undoing the run overwrote it.
