...
The re-base moves a model's whole render frame together: mesh positions and the model's reported bounds (`originalBounds`, `shiftedBounds`) translate by the same delta the new anchor introduces, so a re-based model's camera fit, section-plane range and footprint stay on the geometry instead of trailing the old frame by the anchor distance. The absolute-world data the pipeline publishes alongside — the per-entity `geometryAabb` and the `localToWorld` placement chain, both RTC-invariant by contract — is deliberately left where it is; the new `wasmRtcOffset` is what re-aligns it with the moved vertices.
