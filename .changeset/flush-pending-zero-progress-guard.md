...
Also rejects `+Infinity` and `-Infinity`, not just `NaN`. `NaN` and `+Infinity` both left the `!(x <= cap)` check silently vacuous, same as before; `-Infinity` was worse, because folding it into the running `chunkIndices` total poisoned that total to `-Infinity` too, making the cap vacuous for every mesh after the malformed one, not just that one mesh. Every non-finite `indices.length` now closes the chunk explicitly instead of being added to the running total.
