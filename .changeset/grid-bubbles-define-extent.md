...
Stop grid BUBBLES from framing the camera, and derive the overlay uniform slot count.
...
**Internal, no surface change:** `SECTION_2D_UNIFORM_SLOT_COUNT` is now derived from `SECTION_2D_UNIFORM_SLOT_INDEX` instead of a hand-written `6` (#3342). It sizes the shared overlay uniform buffer; adding a draw site to the index is compiler-forced, bumping the literal beside it was forced by nothing, and a site added without the bump would bind a dynamic offset past the end of the buffer.
