...
`useSymbolicAnnotations` now returns the two content kinds separately (`{ annotation, grid }`), and the viewport uploads each to its own channel, so grid content reaches the policy it was designed for. This is built on top of #3368's single-owner grid pipeline, so there is exactly one grid-line source feeding the split, not two that must agree.
