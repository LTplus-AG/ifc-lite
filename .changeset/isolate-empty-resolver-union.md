...
Leaving the channel untouched is right for a one-shot isolate, and wrong for a channel that REPLACES the view — a BCF viewpoint apply and the anonymized-export preview re-sync both say "the view is now exactly this set", so skipping the assignment leaves the previous viewpoint's (or the previous isolation's) entities on screen labelled as the new one's contents, while the selection channel beside them moves to the real set. Those two call `resolveIsolationIdsForViewSync` instead: same expansion, but always an assignment, falling back to the raw ids on `[]`.

Two isolation call sites are deliberately NOT routed through the helper, each for a reason documented where it lives: `PropertiesPanel.tsx`'s group-member isolate and `SearchModal.filter.tsx`'s filter-result isolate both treat an empty resolve as "these are hidden types that are about to be toggled on", and abort-on-empty would break exactly the case they exist for. `HierarchyPanel.tsx` and `useClash.ts` do not route through it either: the hierarchy tabs isolate ids `treeDataBuilder.ts` already expanded to geometry-bearing members at tree-build time, and a clash pair's ids are geometry-bearing by construction, since detection tests actual mesh triangles.
