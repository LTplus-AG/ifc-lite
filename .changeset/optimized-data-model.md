...
`POST /api/v1/parse/parquet/optimized` now extracts and caches the data model the same way `/parse/parquet` does, written before the response goes out, and reports `data_model_stats` in its `X-IFC-Metadata` header. Previously the data model was produced only by `/parse/parquet`, so a client using only the optimized route never got one.
