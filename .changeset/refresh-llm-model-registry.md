...
**BYOK models.** Anthropic is now Opus 5, Opus 4.8, Fable 5 and Haiku 4.5; OpenAI is the GPT-5.6 family (Sol, Terra, Luna) plus GPT-5.3 Codex, which stays because there is no 5.6 Codex. Sonnet 4.6 and GPT-5.4 are gone with no successor. `claude-haiku-4-5-20251001` becomes `claude-haiku-4-5`: both resolve, but the dated form pins one snapshot while the alias is the id the docs tell you to use, and every other entry here is unsuffixed.
