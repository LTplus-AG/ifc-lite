...
Closes #3875. The `subsetEntityIds` path (what the anonymize export drives) is pinned by its own real-fixture test, including the case where the rescued entity's line cannot be narrowed at all and is withheld: the export now names it in `stats.warnings` instead of letting it disappear silently.
