...
Cost: one hash sweep over the returned triangles per accepted boolean op, plus the hairline sweep only when the strict audit fails. Not measured against the model corpus here; the same predicates already run several times per candidate mesh on the hot analytic path.
