...
Both cards now route the header through a new `setDisplayName(name, kind)` helper (`apps/viewer/src/components/viewer/properties/setDisplayName.ts`), falling back to `Unnamed Property Set` / `Unnamed Quantity Set` when the name is empty. A pset/qset carries no id downstream of the extraction functions that build it, so unlike `treeDataBuilder`'s `getName || "<Type> #<id>"` convention for element rows, the fallback here names only the kind. A named set still renders its real name unchanged.
