...
Adds `Renderer.onPersistentRenderDegradation(listener)` and the exported `RenderDegradationInfo` type: fired once per renderer when frames have degraded *consecutively* well past the transient-retry budget, so a wedged viewport can be surfaced to the user and to error tracking by the host. Any frame that completes resets the run, so a session that failed occasionally and recovered every time never reports. The renderer itself files no telemetry.
