...
`IfcDataStore` gains an optional `resolvedClassifications?: Map<number, ClassificationInfo[]>` field. When present, `extractClassificationsOnDemand` consults it (for the entity itself and, via `IfcRelDefinesByType`, its type) when their count matches the relationship graph, otherwise preserving unresolved markers, so a viewer- or MCP-side IDS check against a server-parsed model can now resolve system- and value-constrained classification facets instead of reporting them as unclassified. The wasm/source-bearing path is unaffected — it still resolves attributes directly from `store.source`.
