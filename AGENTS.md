...
- Elements carrying `data-tour` are interactive-tour anchors. If you move, rename, or delete one, update `apps/viewer/src/lib/tours/anchors.ts` and the referencing steps under `apps/viewer/src/lib/tours/tours/` in the same PR. A step whose anchor no component renders fails `lib/tours/anchors.contract.test.ts`. An anchor that exists but is not mounted in the state the tour reaches still passes CI: it auto-skips at runtime and fires a `tour_step_broken` PostHog event, so give each step a `prepare` (or `panel`) that puts the UI into the state that mounts its anchor. Tour telemetry (`lib/tours/telemetry.ts`) carries only registry ids, enums, and numbers, never file or model names (scrub-safe).
