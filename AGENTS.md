...
- The demo kit (`apps/viewer/public/samples/demo-project*`) is generated by `tools/demo-kit/generate.mts` and committed. Revision B is DERIVED from the committed base bytes (diff matches on GlobalId, so a regenerated base makes compare read as 100% added/deleted). Never hand-edit or regenerate one artifact in isolation; rerun the generator, which self-verifies the IDS/clash/diff invariants.
