...
- CSG kernel routing: a default-features build still runs the **pure-Rust kernel** unless `CSG_MANIFOLD`/`UNION_MANIFOLD` env vars are set at runtime (`csg.rs`) — "Manifold compiled" ≠ "Manifold used". Forensics levers: `CONSOLIDATE_OFF`, `WELD_OFF`, `CLIP_LOG` env; `csg::take_csg_census()`, `GeometryRouter::take_csg_failures()` / `take_host_opening_diagnostics()`.
