...
- Pushing a PR branch does **not** create a Vercel preview: preview deployments are disabled on the Rust+WASM projects (`ifc-lite` viewer, `ifc-lite-viewer-embed`) to save build minutes (`main` still auto-deploys to production). To preview a branch, trigger it on demand: `vercel link` (once per app dir; `.vercel` is gitignored) then `vercel deploy` (`--prod` to promote); `vercel build && vercel deploy --prebuilt` skips the remote WASM compile. Per-project setup and rationale: [`scripts/README-vercel-cost.md`](./scripts/README-vercel-cost.md) section 3d.
