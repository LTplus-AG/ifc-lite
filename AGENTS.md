...
- CSG: the **pure-Rust exact kernel** (`src/kernel/`) is the only CSG kernel on every target since M9 — Manifold C++ and the BSP port are deleted, and the old `CSG_MANIFOLD`/`UNION_MANIFOLD` env escapes no longer exist. Forensics levers: `CONSOLIDATE_OFF`, `WELD_OFF`, `CLIP_LOG` env; `csg::take_csg_census()`, `GeometryRouter::take_csg_failures()` / `take_host_opening_diagnostics()`.
