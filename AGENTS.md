...
- Default colours + style resolution have exactly one home: `ifc_lite_processing::style` (a Rust test fails the build if a duplicate default-colour table or `extract_color_*` helper reappears). New type default = edit that table + extend the mesh fixture. Only sanctioned exception: the 2D drafting palette in `section-2d-overlay.ts` (`PARITY-ALLOW`). (#913)
