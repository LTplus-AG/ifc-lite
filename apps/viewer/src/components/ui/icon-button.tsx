 ...
 * It relies on the app-level `TooltipProvider` (`ViewerLayout`), which lets
 * neighbouring toolbar tooltips skip the open delay; tests wrap it the same way.
 */
  ...
  ({ label, tooltip, tooltipSide, variant = 'ghost', size = 'icon', type = 'button', children, ...props }, ref) => (
    <Tooltip>
      <TooltipTrigger asChild>
        <Button ref={ref} variant={variant} size={size} type={type} aria-label={label} {...props}>
          {children}
        </Button>
      </TooltipTrigger>
