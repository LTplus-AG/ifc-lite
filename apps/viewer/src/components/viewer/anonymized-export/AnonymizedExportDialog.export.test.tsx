...
describe('AnonymizedExportDialog — export', () => {
  it('exports an anonymized .ifc: real download, structural types kept, original identifiers scrubbed', async () => {
    ...
    try {
      for (const [index, surface] of (['classic', 'ribbon', 'palette', 'context_menu'] as const).entries()) {
        render(<AnonymizedExportDialog surface={surface} />);
        ...
        assert.equal(completed.length, index + 1, '#5844: one completion per successful anonymized export');
        ...
        assert.equal(ifcDownload.filename, 'anonymized.ifc', 'default filename must be neutral');
