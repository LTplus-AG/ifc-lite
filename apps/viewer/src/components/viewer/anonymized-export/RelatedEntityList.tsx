...
function buildSections(
  ...
  related: RelatedEntities | null,
  seedsLabel: string,
  groupLabel: (relationship: string, role: string) => string,
): Section[] {
  const sections: Section[] = [{ key: 'seeds', label: seedsLabel, ids: [...seeds] }];
  ...
  for (const group of related.groups) {
    ...
    sections.push({
      ...
      label: groupLabel(group.relationship, group.role),
...
function entityLabel(dataStore: IfcDataStore | null, id: number, fallbackType: string): string {
  const type = dataStore?.entities.getTypeName(id) ?? fallbackType;
    ...
    return (
      <div className="flex items-center gap-2 py-1 text-xs font-medium text-muted-foreground">
        <SectionHeaderCheckbox
          ...
          locked={locked}
          ariaLabel={t('anonymizedExport.relatedList.toggleAllAriaLabel', { label: section.label })}
          onToggle={(next) => {
    ...
    return (
      <label className="flex items-center gap-2 py-0.5 pl-5 text-sm min-w-0">
        ...
        <span className="truncate">
          {entityLabel(dataStore, id, t('anonymizedExport.relatedList.entityFallbackType'))}
        </span>
      </label>
    );
  };
  ...
  if (flatRows.length <= 1) {
    return (
      <div className="text-sm text-muted-foreground py-2">
        {t('anonymizedExport.relatedList.emptyState')}
      </div>
    );
  }
