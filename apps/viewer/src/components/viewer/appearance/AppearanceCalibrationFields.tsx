...
export function AppearanceCalibrationFields({ frame, thumbnailUrl, value, onChange, disabled, onInvalid }: AppearanceCalibrationProps) {
  ...
  return <fieldset disabled={disabled} className="space-y-2">
    ...
    <p className="text-[11px] text-muted-foreground">{t('appearance.calibration.description')}</p>
    ...
    <p id="appearance-calibration-help" className="text-[10px] text-muted-foreground">{t('appearance.calibration.helpText')}</p>
    <label className="block space-y-1 text-[11px]"><span>{t('appearance.calibration.distanceLabel')}</span>
    ...
    {invalid && <p role="status" className="text-[11px] text-muted-foreground">{t('appearance.calibration.invalidMessage')}</p>}
  </fieldset>;
}
