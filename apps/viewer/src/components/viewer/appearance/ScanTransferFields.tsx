...
export function ScanTransferFields({ transfer, disabled }: { transfer: ReturnType<typeof useScanTransfer>; disabled: boolean }) {
  ...
  const update = (key: 'toleranceMetres' | 'texelsPerMetre' | 'maxDistanceMetres' | 'minNormalDot' | 'ambiguityDistanceMetres' | 'maxBehindMetres', value: string) => transfer.setSettings(previous => ({ ...previous, [key]: Number(value), reviewed: key === 'toleranceMetres' ? false : previous.reviewed }));
