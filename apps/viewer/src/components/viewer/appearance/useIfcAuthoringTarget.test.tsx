...
import { prepareTexturedProduct } from '@/lib/appearance/prepare-textured-product';
...
test('both textured creation consumers receive effective IFC bytes from a lazily initialized canonical view #4412', async () => {
  ...
  const prepared = await prepareTexturedProduct('lazy');
  ...
  const edited = await prepareTexturedProduct('lazy');
...
test('selecting a created destination object makes its actual mutation history available after a scan was active #4412', async () => {
  ...
  await prepareTexturedProduct('destination');
  ...
  const restored = await prepareTexturedProduct('destination');
