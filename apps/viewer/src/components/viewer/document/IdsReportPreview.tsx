...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col items-start">
      <span className={`text-[9px] uppercase tracking-wide ${DOCUMENT_PREVIEW_MUTED_TEXT_CLASS}`}>{label}</span>
...
export function IdsReportPreview({ block }: IdsReportPreviewProps) {
  ...
  return (
    <div data-block-ids-report>
      ...
      <div className={`mt-1.5 text-[10px] ${DOCUMENT_PREVIEW_MUTED_TEXT_CLASS}`}>
