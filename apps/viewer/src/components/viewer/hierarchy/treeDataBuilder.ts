 ...
 *  and return its ancestor id chain, using the same one-pass technique as
 *  `filterNodes`: keep, per depth, the flat-array index of the last node
 *  seen there, so any node's ancestors are recoverable in O(1) without a
 *  second walk. Reveals a selection made outside the tree (#5881) — unlike
 *  `filterNodes`, which only needs to know a node is INCLUDED, this needs the
 *  actual ancestor ids in order, to expand a collapsed chain down to the
 *  target. */
