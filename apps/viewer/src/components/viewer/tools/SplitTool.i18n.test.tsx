...
type SplitToolKey = keyof typeof splitToolEn;
const KEYS = Object.keys(splitToolEn) as SplitToolKey[];
...
const mark = (key: SplitToolKey) => `⟦${key}|${splitToolEn[key]}⟧`;
...
describe('Split tool localization (#4918)', () => {
  it('translates the numeric-input panel: mode toggle, length readout, Cut and Snap', () => {
    ...
    for (const key of STATIC_KEYS) {
      ...
      assert.ok(english.has(splitToolEn[key]), `expected "${splitToolEn[key]}" (${key}) to be visible before switching locale`);
    }
  ...
  it('translates the idle hint chip', () => {
    ...
    const english = new Set<string>();
    addReadable(container, english);
    assert.ok(english.has(splitToolEn['splitTool.hintChip']));
