...
/**
 * DXF underlay → 2D drawing space (issue #1782).
 *
 ...
 * X. This hook applies that mapping (the same shift the profile-projection
 * path uses in useDrawingGeneration), then the per-underlay placement
 * (offset/rotation/scale in drawing space), and filters by underlay/layer
 * visibility. Underlays are plan content, so anything but a cardinal
 * 'down' section yields no data.
 */
 ...
 * Mirrors useDrawingGeneration's profile path: the WASM RTC offset is in
 * IFC Z-up; the TS `originShift` is already Y-up, so its IFC Y is `-z`.
 */
export function dxfWorldShift(coordinateInfo: GeometryResult['coordinateInfo'] | undefined): { x: number; y: number } {
  const rtc = coordinateInfo?.wasmRtcOffset;
  if (rtc) return { x: rtc.x, y: rtc.y };
  const shift = coordinateInfo?.originShift;
  if (shift) return { x: shift.x, y: -shift.z };
  return { x: 0, y: 0 };
}
