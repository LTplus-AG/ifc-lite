...
test('cooperative preparation preserves real Convento broad Apply and repeat Apply (#4336)', async t => {
  ...
  for (let pass = 0; pass < 2; pass++) {
    ...
    let yields = 0;
    const { prepared, applied } = await prepareAppearanceEntities(editor, view, plan, revision, {
      yieldTask: () => new Promise(resolve => { yields++; setTimeout(resolve, 0); }),
    });
    assert.ok(yields > 0);
    assert.equal(applied.created.length, plan.created.length);
