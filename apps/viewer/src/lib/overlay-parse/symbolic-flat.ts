 ...
 * `polylineToSegments`) and the multi-line text split stay on the main thread.
 */
...
export function collectFlatSymbolic(
  ...
  for (let i = 0; i < collection.textCount; i++) {
    ...
    try {
      ...
      // decodes at the parse boundary (`AttributeValue::from_token`) — so this
      // side only splits it into lines.
