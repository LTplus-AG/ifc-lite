...
import type { PageAppearancePlan, PageAppearanceRequest, AppearanceCatalog, AppearanceCatalogRequest, AppearancePlan, AppearanceRequest, AppearanceWorkerRequest, AppearanceWorkerResponse } from '../lib/appearance/planner-types.js';
...
if (isWorkerScope) {
  self.onmessage = async (event: MessageEvent<AppearanceWorkerRequest>) => {
    ...
    if (!job || (job.type !== 'plan' && job.type !== 'catalog' && job.type !== 'page-plan')) return;
    try {
