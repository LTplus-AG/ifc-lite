...
| F8 / F3 | Supported PDF vectors to IfcAnnotation | Active [#4406](https://github.com/LTplus-AG/ifc-lite/issues/4406). [Real vector/control investigation](evidence/pdf-vectors/README.md) pins the decoder and exposes the missing standalone 3D annotation processor; no conversion action is enabled yet. Compare curves, fills, text, fonts and line styles against the page raster in 2D and 3D and in an independent IFC reader. Unsupported operators block exact conversion or require explicit acceptance of partial output. |
