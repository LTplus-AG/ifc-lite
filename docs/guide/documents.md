...
| **Validation results table** | Rows from the Validation panel's last report — `Failed`, `Passed`, `All` entities, or `Sets` (one row per uniqueness/aggregate check), optionally filtered to one rule; which columns print (rule, result, entity type, name, GlobalId, model, actual, expected, reason, set, members); title, caption | Paginated by row, the header repeated on every page, a row never split; capped at 2 000 rows with a truncation note. Resolved against the *live* report, not a saved snapshot — a stale or cleared report prints a placeholder instead of yesterday's rows |
...
The file is `version: 3` (#5138: the validation-results table block; #4940: chart height, the `Half` width and the spacer block). A `version: 1` or `version: 2` file opens and re-saves as `version: 3` automatically — nothing to do; an older ifc-lite build refuses a `version: 3` file loudly rather than silently misreading it. Out of scope for now: text in columns or more than two columns per row, arbitrary font family/size/colour, a per-chart legend position, page margins, and drag-resize.
