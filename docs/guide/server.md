...
| `/api/v1/parse/parquet/optimized` | POST | Optimized Parquet (~50x smaller); also produces the data model, like `/parse/parquet` |
...
# already applied) so a hit warmed before that change re-parses instead of
# replaying a geometry-only entry with no data model behind it.
