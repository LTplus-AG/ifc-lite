...
<!-- numeral-src: 49 :: none - B4.2's flagged-schedule denominator. That bet commits no scorecard, so nothing backs it; negative-bound 2026-08-01 after a union hit made it read as backed by an unrelated sampled coordinate in B4.4's cross-check-points.json, reached only through the checker's x100 scale for a bare numeral. -->
<!-- numeral-ok: 40.82%, 8.78% :: B4.2's figures, emitted by that bet's
...
<!-- numeral-ok: 28.2% :: the lower endpoint of the reviewer's own Wilson 95%
     ...
     any artifact: it is an analysis OF the bet's numbers, not one of them. -->
<!-- numeral-src: 54.7% :: none - the upper endpoint of that same Wilson
     interval, and unbacked for the same reason. Negative-bound 2026-08-01:
     unlike its lower endpoint it went STALE, because a sampled placement
     coordinate in B4.4's cross-check-points.json is 0.5471666780330123 and the
     percent scale turns that into a match. An interval this document computed
     must not acquire provenance from a coordinate. -->
