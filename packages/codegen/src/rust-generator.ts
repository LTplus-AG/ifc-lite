...
interface RustGeneratorOptions {
...
export function generateRust(
  ...
  return {
    typeIds: generateTypeIdConstants(schema, options),
    schema: generateIfcTypeEnum(schema, options),
  };
}
 ...
 */
function generateTypeIdConstants(schema: ExpressSchema, options: RustGeneratorOptions): string {
...
`;
  ...
  return options.cratePrivate ? `${code.trimEnd()}\n` : code;
}

/** Generate the main IfcType enum. */
function generateIfcTypeEnum(schema: ExpressSchema, options: RustGeneratorOptions): string {
  const visibility = options.cratePrivate ? 'pub(crate)' : 'pub';
  let code = `// This Source Code Form is subject to the terms of the Mozilla Public
// License, v. 2.0. If a copy of the MPL was not distributed with this
// file, You can obtain one at https://mozilla.org/MPL/2.0/.
...
${options.cratePrivate ? '#![allow(dead_code, clippy::enum_variant_names)] // Full EXPRESS names are required; registry consumers currently use only name lookup and attribute slots.\n\n' : ''}use std::fmt;
