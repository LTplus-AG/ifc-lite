...
import { generateUnknownIfcType } from './rust-unknown-ifc-type.js';
import { generateTypeIdConstants } from './rust-type-id-generator.js';
...
export function generateRust(
  ...
  return {
    typeIds: generateTypeIdConstants(typeUniverse, cratePrivate, categorizeEntities),
...
${cratePrivate ? '#![allow(dead_code, clippy::enum_variant_names)] // Full EXPRESS names are required; registry consumers currently use only name lookup and attribute slots.\n\n' : ''}use std::fmt;

${generateUnknownIfcType(visibility)}
/// IFC Entity Types
///
...
`;
  ...
  code += `    /// Unknown/unrecognized IFC type with its normalized keyword.
...
`;
  ...
  code += `            Self::Unknown(unknown) => unknown.id(),
        }
    }
...
`;
  ...
  code += `            Self::Unknown(unknown) => unknown.as_str(),
        }
    }
...
`;
  ...
  for (const entity of typeUniverse.entities) {
    code += `            Self::${entity.name} => Some("${entity.name.toUpperCase()}"),\n`;
  }

  code += `            Self::Unknown(_) => None,
        }
    }
