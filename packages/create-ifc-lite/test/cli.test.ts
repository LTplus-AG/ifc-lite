...
import { mkdtempSync, mkdirSync, rmSync, existsSync, writeFileSync, chmodSync, readdirSync, readFileSync, realpathSync } from 'node:fs';
...
describe('template flag parsing', () => {
  ...
  // Was a pinned KNOWN DEFECT: the guard used `t in TEMPLATES`, and `in` walks
  // the prototype chain, so every Object.prototype key passed it, matched no
  // branch, and silently produced the `basic` scaffold. The guard is now
  // `Object.hasOwn`, and the template map dispatches directly.
  for (const protoKey of ['toString', 'constructor', 'valueOf', 'hasOwnProperty']) {
    it(`rejects --template ${protoKey} instead of silently scaffolding basic`, () => {
      const { status, stderr } = run(['proj', '--template', protoKey]);
      expect(stderr).toContain('Invalid template');
      expect(status).toBe(1);
      expect(existsSync(join(workDir, 'proj'))).toBe(false);
    });
  }
  ...
  // Unrecognised flags used to be skipped silently, which made a typo'd
  // `my-app --templat react` scaffold a project literally named "react": the
  // flag was dropped and its value taken as the positional.
  it('rejects an unknown flag instead of scaffolding under its value', () => {
    const { status, stderr } = run(['my-app', '--templat', 'react']);
    expect(stderr).toContain('Unknown option: --templat');
    expect(status).toBe(1);
    expect(existsSync(join(workDir, 'react'))).toBe(false);
    expect(existsSync(join(workDir, 'my-app'))).toBe(false);
  });

  it('rejects an unknown flag on its own', () => {
    const { status, stderr } = run(['--verbose']);
    expect(stderr).toContain('Unknown option: --verbose');
    expect(status).toBe(1);
    expect(existsSync(join(workDir, 'my-ifc-app'))).toBe(false);
  });

  // A second positional used to silently override the first, so `create-ifc-lite
  // a b c` produced a project called "c".
  it('rejects a second positional argument', () => {
    const { status, stderr } = run(['first', 'second']);
    expect(stderr).toContain('Unexpected argument: second');
    expect(status).toBe(1);
    expect(existsSync(join(workDir, 'first'))).toBe(false);
    expect(existsSync(join(workDir, 'second'))).toBe(false);
  });
});

describe('version', () => {
  // `--version` used to fall through the flag branch and scaffold `my-ifc-app`.
  for (const flag of ['--version', '-v']) {
    it(`prints the package version and creates nothing for ${flag}`, () => {
