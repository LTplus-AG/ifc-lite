...
function gatherBodyFootprintPoints(
  ...
  // Dedup coincident points (within 0.1 mm). A closed profile polyline repeats
  // its first point and a mesh projects many shared vertices onto the same XY —
  // both pull the PCA *centroid* (and so `principalAxisCentreline`'s axis) off
  // the true wall mid by a couple of cm. Unique points give an unbiased centroid
  // → the derived axis lands on the wall centreline.
  const uniq: Vec2[] = [];
  for (const p of pts) {
