 ...
 * (C0 plus DEL) each become ONE space -- per character, not per run -- so a
 * value cannot inject a line break into the line-oriented STEP output. The `+`
 * this used to carry made `"a\t\t\tb"` serialise as `'a b'` here and `'a   b'`
 * in Rust, a real disagreement between two halves whose doc comments each
 * claimed to match the other (#3284). 6.3.3.4 mandates neither, so the
 * tie-break is that preserving the count keeps information a collapse throws
 * away, and it is what `step_text::escape` already did.
 *
 * 6.3.3.4 also restricts plain-text bytes to the "basic graphic" range 32-126;
 * everything else is a control directive (`\X\HH`, `\X2\HHHH\X0\`,
