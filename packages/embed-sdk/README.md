...
- Appearance: `setColors`, `resetColors`, `setTheme`, `setTypeVisibility` (every flag in `TypeVisibilityFlags`). `hideTypes` wins over these where they overlap, and toggle state persists per viewer, so send `setTypeVisibility` on load if you need a deterministic starting state.
