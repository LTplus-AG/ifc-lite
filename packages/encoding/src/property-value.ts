 ...
 *
 * It deliberately does NOT call {@link decodeIfcString}. Every producer of a
 * property value decodes exactly once, at parse time: the TypeScript path via
 ...
 * \u2014 `decodeIfcString` collapses `\\` to `\`, so an authored UNC path
 * `\\server\share` would render as `\server\share` (#2323 follow-up).
 *
