  ...
  for (const [schema, fixture, unitPrefix] of [['IFC2X3', 'ara3d/duplex.ifc', '$'], ['IFC4', 'ara3d/IfcOpenHouse_IFC4.ifc', '.MILLI.']] as const) {
    it(
      ...
      async () => {
        ...
        // `ContainedInStructure` and the set's (IFC2X3) `PropertyDefinitionOf`
        // / (IFC4) `DefinesOccurrence` are SET [0:1], so writing the later
        // models' rels too gives each a second and a third.
        ...
        // Not vacuous: B's definer is written and C's rels are what the check is about.
        expect(merged).toContain('3Defs5774Repeated0000b');
        expect(merged).not.toContain('3Defs5774Repeated0000c');
