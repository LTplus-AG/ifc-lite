...
/**
 * A merge never states a single-valued inverse twice (#5923, #5774).
 *
...
describe('one definer per property set (#5774)', () => {
  const pset = (n: number, members: string) => [
    `#${n}=IFCPROPERTYSINGLEVALUE('Mark',$,IFCLABEL('m'),$);`,
    `#${n + 1}=IFCPROPERTYSET('${guid('ps')}',$,'Pset_WallCommon',$,(#${n}));`,
    `#${n + 2}=IFCRELDEFINESBYPROPERTIES('${guid(`rp${members.length}`)}',$,$,$,(${members}),#${n + 1});`,
  ];

  for (const schema of ['IFC2X3', 'IFC4'] as const) {
    it(`${schema}: a GlobalId-unified property set keeps one IfcRelDefinesByProperties`, async () => {
      const a = [PROJECT('a'), WALL(3, 'w1', schema), ...pset(4, '#3')];
      const b = [PROJECT('b'), WALL(3, 'w1', schema), ...pset(4, '#3').map(l => l.replace(guid('rp2'), guid('rpb')))];
      const content = await merge([await model('a', a, schema), await model('b', b, schema)], { schema });
      expect(rels(content, 'IFCRELDEFINESBYPROPERTIES', 4, 5)).toEqual([
        { list: [idOf(content, guid('w1'))], single: idOf(content, guid('ps')) },
      ]);
    });
  }

  it('a later model that shares the set with a new element folds it into the first definer, so no element loses it', async () => {
    const a = [PROJECT('a'), WALL(3, 'w1'), ...pset(4, '#3')];
    const b = [PROJECT('b'), WALL(3, 'w1'), WALL(10, 'w2'), ...pset(4, '#3,#10')];
    const content = await merge([await model('a', a), await model('b', b)]);
    expect(rels(content, 'IFCRELDEFINESBYPROPERTIES', 4, 5)).toEqual([
      { list: [idOf(content, guid('w1')), idOf(content, guid('w2'))], single: idOf(content, guid('ps')) },
    ]);
    // It is A's rel that carries both.
    expect(content).toContain(guid('rp2'));
    expect(content).not.toContain(guid('rp6'));
  });
});
