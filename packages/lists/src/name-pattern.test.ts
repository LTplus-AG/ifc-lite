...
describe('ReDoS guard', () => {
  it('rejects a catastrophic-backtracking pattern quickly, without ever running it', () => {
    ...
    // seconds (measured ~9s for n=30 on this machine); a bound well under
    // that proves the guard fired BEFORE any matching was attempted, not
    // that a slow match happened to finish fast.
  ...
  it('rejects a pattern over the length cap, naming the reason', () => {
    ...
    expect(() => compileNameMatcher(long)).toThrow(/exceeds the 256-character limit \(300 characters\)/);
  });
  ...
  it('unsafeNamePatternReason is pure and never throws, even on a dangerous source', () => {
    ...
    expect(unsafeNamePatternReason('a'.repeat(300))).toMatch(/exceeds the 256-character limit/);
  });
});
