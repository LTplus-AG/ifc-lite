...
import { FoundationAuthenticationError } from './errors.js';
...
describe('requestPasswordToken', () => {
  it('posts a form-encoded password grant and parses the token set', async () => {
    ...
    const token = await requestPasswordToken({
      tokenUrl: 'https://host/foundation/oauth2/token',
    ...
    expect(captured?.url).toBe('https://host/foundation/oauth2/token');
  ...
  it('throws FoundationAuthenticationError with the RFC 6749 error fields', async () => {
    ...
    expect(error).toBeInstanceOf(FoundationAuthenticationError);
    const authError = error as FoundationAuthenticationError;
...
describe('exchangeAuthorizationCode', () => {
  it('posts the code, redirect_uri, PKCE verifier, and client credentials', async () => {
    ...
    const token = await exchangeAuthorizationCode({
      tokenUrl: 'https://host/token',
      code: 'auth-code-1',
      redirectUri: 'https://viewer.example/oauth/callback',
    ...
    expect(form.get('redirect_uri')).toBe('https://viewer.example/oauth/callback');
...
describe('registerClient', () => {
  it('posts the JSON registration and returns the minted client', async () => {
    ...
    const client = await registerClient({
      registrationUrl: 'https://host/foundation/oauth2/reg',
  ...
  it('surfaces registration rejections and rejects responses without a client_id', async () => {
    const rejecting: FetchLike = async () => jsonResponse({ message: 'registration disabled' }, 403);
    await expect(
      registerClient({ registrationUrl: 'https://host/reg', clientName: 'x', fetchFn: rejecting }),
    ).rejects.toThrow('registration disabled');
    const empty: FetchLike = async () => jsonResponse({});
    await expect(
      registerClient({ registrationUrl: 'https://host/reg', clientName: 'x', fetchFn: empty }),
