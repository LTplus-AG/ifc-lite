...
export function mergeInheritedPropertySets<T extends NamedPropertySet>(
    ...
    for (let i = 0; i < merged.length; i++) {
        if (!byName.has(merged[i].name)) byName.set(merged[i].name, i);
    }

    for (const inherited of inheritedSets) {
        const index = byName.get(inherited.name);
        if (index === undefined) {
            merged.push(inherited);
            byName.set(inherited.name, merged.length - 1);
            continue;
        }
        ...
        // `as T`: the spread reproduces every field of `own` and replaces only
        // `properties` with a superset of the same element type, but TypeScript
        // cannot narrow a spread of an unresolved generic back to that generic.
