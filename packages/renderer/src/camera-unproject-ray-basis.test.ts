 ...
 * `normalize(cross(forward, up))`. `MathUtils.normalize` floors on
 * `len < 1e-10`, which is false for NaN, so it divided by NaN instead of
 * falling back — and Infinity got through the other way, since
...
/**
 * Both non-finite flavours in each component, because they do not behave
 * alike: NaN fails the `len < 1e-10` floor by being incomparable, Infinity
 * passes it and then poisons the scale. A guard written `!Number.isNaN(...)`
 * would satisfy half of this list.
 */
