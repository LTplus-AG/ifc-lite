...
/**
 * Which post passes a frame runs (#5384). Ambient occlusion reads only the
 * depth attachment, so it no longer forces the object-id attachment to be
 * stored: the renderer keys that store on `separationLines` alone.
 */
describe('livePostEffects (#5384)', () => {
  ...
  it('pauses every pass while the interaction governor has effects off', () => {
    ...
    assert.deepEqual(livePostEffects(ve, false), { ambientOcclusion: null, separationLines: false });
    assert.deepEqual(livePostEffects(ve, true), { ambientOcclusion: 'low', separationLines: true });
  });

  it('honours the global switch', () => {
    const ve = resolve({ enabled: false, contactShading: { quality: 'high' }, separationLines: { enabled: true } });
    assert.deepEqual(livePostEffects(ve, true), { ambientOcclusion: null, separationLines: false });
  });

  it('keeps separation lines independent of AO', () => {
    const ve = resolve({ contactShading: { quality: 'low' }, separationLines: { enabled: false } });
    assert.deepEqual(livePostEffects(ve, true), { ambientOcclusion: 'low', separationLines: false });
    const off = resolve({ contactShading: { quality: 'off' }, separationLines: { enabled: true, quality: 'off' } });
    assert.deepEqual(livePostEffects(off, true), { ambientOcclusion: null, separationLines: false });
  });
});
