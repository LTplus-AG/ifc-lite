...
/** Posts one message the way the static callback page does. */
...
function callbackMessage(state: string, code = 'code-1'): unknown {
  return {
    ...
    state,
    url: `https://app.example.com/oauth/dropbox/callback?code=${code}&state=${state}`,
  };
}
...
describe('waitForOAuthCallback', () => {
  it('resolves with the callback URL the page broadcast', async () => {
    ...
    await expect(pending).resolves.toBe(
      'https://app.example.com/oauth/dropbox/callback?code=code-1&state=state-a',
    );
  });
  ...
  it('rejects with the caller-supplied message when nothing ever arrives', async () => {
    ...
    ).rejects.toThrow('Dropbox sign-in timed out');
  });
});
