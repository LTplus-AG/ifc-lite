...
/// type actually derives from `IfcRoot`. A thin positional wrapper around
/// [`extract_leading_guid`] (`rooted_type.rs`), which owns the actual
/// rootedness check (schema-driven `IfcType::is_subtype_of` plus
/// `LEGACY_ROOTED_TYPES` for the IFC2X3/IFC4-only rooted types the
/// IFC4X3-only generated schema doesn't know) and the positional scan.
///
/// This module used to carry its own byte-for-byte copy of that same check
/// -- exactly the "two lists that must agree, no gate" drift shape #3015 is
/// about, just one file over instead of one language over. Mirrors
/// `extractGlobalIdFast` in `packages/export/src/merged-exporter.ts`, which
/// is likewise positional (skips only whitespace after `(`) and, since
/// #3030, likewise schema-driven rather than a denylist.
fn leading_guid(line: &[u8], type_name: &str) -> Option<String> {
    extract_leading_guid(type_name, line)
}
