...
/// #5774: in IFC2X3 a property set has one `IfcRelDefinesByProperties`. A later
/// rel naming a unified one is not written; objects only it names are folded
/// into the first rel's list once the file is written.
#[test]
fn a_later_pset_definer_is_folded_into_the_first_in_ifc2x3() {
    let mut parents = ParentClaims::new(true);
    let first = "#4=IFCRELDEFINESBYPROPERTIES('a',$,$,$,(#10),#3);".to_string();
    assert_eq!(parents.claim("IFCRELDEFINESBYPROPERTIES", first.clone(), false), Some(first));
    ...
    let third = "#44=IFCRELDEFINESBYPROPERTIES('c',$,$,$,(#30,#50),#3);".to_string();
    assert_eq!(parents.claim("IFCRELDEFINESBYPROPERTIES", third, true), None);
    let mut out = "DATA;\n#4=IFCRELDEFINESBYPROPERTIES('a',$,$,$,(#10),#3);\n#10=IFCWALL('w',$,$,$,$,$,$,$);\n".to_string();
    assert!(parents.apply_folds(&mut out).is_empty());
    assert_eq!(
        out,
        "DATA;\n#4=IFCRELDEFINESBYPROPERTIES('a',$,$,$,(#10,#30,#50),#3);\n#10=IFCWALL('w',$,$,$,$,$,$,$);\n"
    );
}

/// IFC4 relaxes the property-set side to `DefinesOccurrence : SET [0:?]`.
#[test]
fn a_later_pset_definer_is_kept_in_ifc4() {
    let mut parents = ParentClaims::new(false);
    let first = "#4=IFCRELDEFINESBYPROPERTIES('a',$,$,$,(#10),#3);".to_string();
    parents.claim("IFCRELDEFINESBYPROPERTIES", first, false);
    let later = "#24=IFCRELDEFINESBYPROPERTIES('b',$,$,$,(#10,#30),#3);".to_string();
    assert_eq!(parents.claim("IFCRELDEFINESBYPROPERTIES", later.clone(), true), Some(later));
}

/// `IfcRelOverridesProperties.WR1` allows one object, so nothing folds into
/// one, and neither across rel types: the later rel is withheld and reported.
#[test]
fn never_folds_into_an_override_or_across_rel_types() {
    for owner_type in ["IFCRELOVERRIDESPROPERTIES", "IFCRELDEFINESBYPROPERTIES"] {
        let mut parents = ParentClaims::new(true);
        let first = format!("#4={owner_type}('a',$,$,$,(#10),#3,(#2));");
        parents.claim(owner_type, first.clone(), false);
        let later = "#24=IFCRELOVERRIDESPROPERTIES('b',$,$,$,(#30),#3,(#2));".to_string();
        assert_eq!(parents.claim("IFCRELOVERRIDESPROPERTIES", later, true), None, "{owner_type}");
        let mut out = format!("{first}\n");
        let warnings = parents.apply_folds(&mut out);
