...
/// Escape a STEP string literal body (double single-quotes and reverse
/// solidus; drop control chars).
///
/// Both doublings are mandated by ISO 10303-21 and both are undone on read
/// (`''` by `AttributeValue::from_token`, `\\` by `decode_ifc_string`), so
/// omitting either here would turn a parse fix into a write bug: a value
/// holding one backslash would come back holding none.
