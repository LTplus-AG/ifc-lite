...
/// clamped into [0,1] (non-finite → mid-grey/opaque), then RGB decoded through
/// [`srgb_to_linear`], the decode the glTF writer applies. Alpha is opacity and
/// is not decoded. This used to clamp only, so mid-grey `0.5` reached USD as
/// `0.5` and glTF as `0.214`: the same element in two colours.
pub(super) fn linear_color(c: [f32; 4]) -> [f32; 4] {
    let f = |v: f32, d: f32| if v.is_finite() { v.clamp(0.0, 1.0) } else { d };
    [
        srgb_to_linear(f(c[0], 0.8)),
        srgb_to_linear(f(c[1], 0.8)),
        srgb_to_linear(f(c[2], 0.8)),
        f(c[3], 1.0),
    ]
}
