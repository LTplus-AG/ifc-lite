...
#[cfg(test)]
mod tests {
    //! Smoke tests for the FFI boundary itself — pointer validation, error
    //! codes, the parse→serialize→free round trip, and the `opening_filter_mode`
    //! mapping. Geometry correctness is covered by the `geometry`/`processing`
    //! crates; here we only assert the C ABI contract documented above.
