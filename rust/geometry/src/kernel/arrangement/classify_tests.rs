...
// ---- TEMPORARY debug for #3353 divergence pin; will be reverted ----
#[test]
fn debug_repro_step3b_origin_split_sweep_261() {
    use crate::mesh::Mesh;
    use nalgebra::{Point3 as NPoint3, Rotation3, Unit, Vector3 as NVector3};
    use crate::kernel::arrangement::{arrange, BoolOp};
    use super::super::classify::{boolean_vids, c_on_or_near_a, on_surface_normal, centroid as centroid_fn};
    ...
    let a: Vec<crate::kernel::arrangement::Tri> =
        crate::kernel::mesh_bridge::orient_outward(crate::kernel::mesh_bridge::mesh_to_tris(&a_mesh));
    let b: Vec<crate::kernel::arrangement::Tri> =
        crate::kernel::mesh_bridge::orient_outward(crate::kernel::mesh_bridge::mesh_to_tris(&b_mesh));

    let arr = arrange(&a, &b);
    let kept = boolean_vids(&arr, &a, &b, BoolOp::Union);
    let kept_set: std::collections::HashSet<[u32; 3]> = kept.iter().copied().collect();

    let mut sorted3 = |t: [u32; 3]| { let mut v = t; v.sort(); v };

    eprintln!("--- A sub-triangles touching 13/14 ---");
    let bvh_a_dummy = crate::kernel::broadphase::Bvh::build(&a);
    let a_band = crate::kernel::near_band::NearBand::from_tris(&a);
    let mut scratch = Vec::new();
    for tri in &arr.tris_a {
        if tri.contains(&13u32) || tri.contains(&14u32) {
            let c = centroid_fn(&arr, *tri);
            let coincident_with_b = on_surface_normal(c, &b).is_some();
            let is_kept = kept_set.contains(tri);
            eprintln!(
                "A tri={:?} sorted={:?} kept={} on_surface_normal(c,b).is_some()={}",
                tri, sorted3(*tri), is_kept, coincident_with_b
            );
        }
    }
    eprintln!("--- B sub-triangles touching 13/14 ---");
    for tri in &arr.tris_b {
        if tri.contains(&13u32) || tri.contains(&14u32) {
            let c = centroid_fn(&arr, *tri);
            let coincident_with_a = c_on_or_near_a(c, &a, &bvh_a_dummy, &a_band, &mut scratch);
            // is it kept, accounting for the Difference-only winding flip (union: no flip)
            let is_kept = kept_set.contains(tri) || kept_set.contains(&[tri[0], tri[2], tri[1]]);
            eprintln!(
                "B tri={:?} sorted={:?} kept={} c_on_or_near_a(c,a)={}",
                tri, sorted3(*tri), is_kept, coincident_with_a
            );
        }
    }
    panic!("STEP3b done - see stderr");
}
