...
//! stack-allocated 1024-bit integers (`bnum::BInt<16>`) — no heap, no GCD. Every
//! op is CHECKED: any overflow (degenerately huge coords) OR an off-grid coord
//! returns `None`, falling through to the BigRational tier. So the result is
...
/// f64 coord → exact scaled integer (`coord · 2^16`). `None` if the coord is not
/// on the 2^16 grid (would lose exactness) or too large to scale into `i64`.
#[inline]
...
/// LPI λ-construction (exact, scaled), mirroring `rational::lpi_lambda`.
...
/// TPI λ-construction (exact, scaled), mirroring `rational::tpi_lambda` (Cramer).
...
/// `orient2d_2i` (two implicit, one explicit) — mirrors `rational::orient2d_2i`.
...
/// `orient2d_3i` (three implicit) — mirrors `rational::orient2d_3i`.
...
/// `orient2d` with one implicit point + two explicit — mirrors
/// `rational::indirect_orient2d`.
...
/// Lexicographic compare on axis `k` — mirrors `rational::cmp_axis`.
...
/// Lexicographic compare of two points — mirrors `rational::cmp_lex`.
...
/// Exact sign of `(a − b)·u` — mirrors `rational::cmp_along` (`lambda_of` already
/// handles the Explicit `(coord, 1)` case).
...
/// `orient3d` with one implicit (LPI/TPI) point + three explicit — mirrors
/// `rational::indirect_orient3d`.
