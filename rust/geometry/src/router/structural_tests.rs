...
fn midpoints(mesh: &crate::Mesh) -> ([f64; 3], [f64; 3]) {
    ...
    let mid = |a: [f64; 3], b: [f64; 3]| [(a[0] + b[0]) / 2., (a[1] + b[1]) / 2., (a[2] + b[2]) / 2.];
...
fn straight_edge_meshes_to_a_ribbon_with_correct_endpoints() {
    ...
    assert!(!mesh.is_empty(), "structural curve member must mesh to a ribbon");
    ...
    for axis in 0..3 {
        assert!((start[axis] - [0., 0., 0.][axis]).abs() < 1e-6, "start: {start:?}");
        assert!((end[axis] - [10., 0., 0.][axis]).abs() < 1e-6, "end: {end:?}");
    }
}
...
fn edge_parallel_to_up_axis_uses_the_fallback_perpendicular() {
    ...
    let (start, end) = midpoints(&mesh);
    assert!((start[2] - 0.).abs() < 1e-6 && (end[2] - 10.).abs() < 1e-6, "{start:?} {end:?}");
    ...
    assert!(width_sq > 1e-6, "ribbon collapsed to a zero-width line: {a:?} vs {b:?}");
}
...
fn zero_length_edge_meshes_empty_instead_of_producing_nan_geometry() {
    ...
    let mesh = router.process_element(&entity, &mut decoder).unwrap();
    assert!(mesh.is_empty(), "a zero-length edge has no direction to build a ribbon from");
}
