...
impl SpacePlate {
    ...
    pub fn build(segments: &[InputSegment], options: BuildOptions) -> Self {
        ...
        // Straighten near-axial walls onto the model's own orthogonal frame
        // first: derived centrelines carry sub-degree skew (e.g. a PCA-of-
        // footprint axis tilted ~0.5°), so corners sit on the *tilted* axis, not
        // the true wall line. This snaps them clean (creating exact collinearity
        // the prune below can then dissolve).
        plate.regularize_axes(3.0_f64.to_radians(), options.snap_tolerance.min(0.15));
    ...
    /// Snap vertices onto the model's own orthogonal frame so near-axial walls
    /// become exactly axis-aligned — collinear runs collapse onto one grid line
    /// and corners land where the walls truly cross. The derived arrangement's
    /// centrelines drift a few cm across a wall's length (sub-degree PCA tilt +
    /// the corner-snap picking a representative vertex), which a uniform offset
    /// hides but the eye still reads as "nodes not on the wall axis".
    ///
    /// TOPOLOGY-PRESERVING: it only moves vertex *positions*, never adding,
    /// removing, or merging edges, so it cannot collapse a room — unlike snapping
    /// the input *segments*, which changes which walls intersect. A wall whose
    /// direction is beyond `angle_tol` of every frame axis (a genuinely angled
    /// wall) imposes no constraint and keeps its geometry, so a non-orthogonal
    /// building is left alone. A coordinate group is snapped only when its spread
    /// is below `max_shift`, so a long cumulative drift is never force-collapsed
    /// and a move is always bounded by `max_shift` (keeps thin features apart).
    fn regularize_axes(&mut self, angle_tol: f64, max_shift: f64) {
        ...
        // 1. Dominant frame: length-weighted circular mean of 4×edge-angle
        //    (period 90°), so it locks onto the building's grid at any rotation.
        let mut sx = 0.0;
        let mut sy = 0.0;
        let mut edges: Vec<(usize, usize)> = Vec::new();
        for i in 0..self.half_edges.len() {
            let h = &self.half_edges[i];
            if !h.alive || h.twin.0 < i as u32 {
                continue; // one direction per undirected edge
            }
            let a = h.origin.0 as usize;
            let b = self.dest(HalfEdgeId(i as u32)).0 as usize;
            let (pa, pb) = (self.vertices[a].pos, self.vertices[b].pos);
        ...
        let theta = sy.atan2(sx) / 4.0;
        let u = [theta.cos(), theta.sin()];
        let v = [-theta.sin(), theta.cos()];
        // Frame coords (pu along the frame x, pv along the frame y) per vertex.
        let uv: Vec<[f64; 2]> = self
            .vertices
            .iter()
            .map(|vx| [vx.pos[0] * u[0] + vx.pos[1] * u[1], vx.pos[0] * v[0] + vx.pos[1] * v[1]])
            .collect();

        // 2. Group vertices that should share a coordinate: a u-axial (horizontal
        //    in frame) wall ties its endpoints' v; a v-axial wall ties their u.
        let n = self.vertices.len();
        let mut pv_set: Vec<usize> = (0..n).collect(); // share v
        let mut pu_set: Vec<usize> = (0..n).collect(); // share u
        let quarter = std::f64::consts::FRAC_PI_2;
        for &(a, b) in &edges {
            let ang = (uv[b][1] - uv[a][1]).atan2(uv[b][0] - uv[a][0]);
            let k = (ang / quarter).round();
            if (ang - k * quarter).abs() > angle_tol {
                continue; // genuinely angled — no axis constraint
            }
            if k.rem_euclid(2.0) < 0.5 {
                union(&mut pv_set, a, b); // horizontal → equal v
            } else {
                union(&mut pu_set, a, b); // vertical → equal u
            }
        }

        // 3. Per group: snap the shared coord to the group mean, but only when the
        //    spread is small enough to really be one line (else leave it).
        let snap_axis = |sets: &mut [usize], axis: usize| -> Vec<Option<f64>> {
            let mut members: std::collections::HashMap<usize, Vec<usize>> = std::collections::HashMap::new();
            for i in 0..n {
                if self.vertices[i].alive {
                    let r = find(sets, i);
            ...
            for group in members.values() {
                ...
                if hi - lo > max_shift {
                    continue; // not one line — don't force a big collapse
                }
        ...
        let target_v = snap_axis(&mut pv_set, 1);
        let target_u = snap_axis(&mut pu_set, 0);

        // 4. Write the snapped frame coords back to world — but only when the
        //    move is meaningful, so an already-clean plate (and the round-trip's
        //    ~1e-17 frame noise) stays bit-for-bit unchanged.
        for i in 0..n {
            if !self.vertices[i].alive {
                continue;
            }
            let pu = target_u[i].unwrap_or(uv[i][0]);
            let pv = target_v[i].unwrap_or(uv[i][1]);
            let np = [pu * u[0] + pv * v[0], pu * u[1] + pv * v[1]];
            let old = self.vertices[i].pos;
            if (np[0] - old[0]).hypot(np[1] - old[1]) > EPS {
                self.vertices[i].pos = np;
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn regularize_straightens_a_slightly_tilted_wall() {
        // A 4×3 room whose right wall is tilted ~1° (top-right corner 0.08 m off
        // — beyond the snap so the arrangement keeps the tilt). After build the
        // wall is vertical: both right corners share an x, so it's a clean rect.
        let segs = vec![
            InputSegment::new([0.0, 0.0], [4.0, 0.0], Some(1)),
            InputSegment::new([4.0, 0.0], [4.08, 3.0], Some(2)), // tilted right wall
            InputSegment::new([4.08, 3.0], [0.0, 3.0], Some(3)),
            InputSegment::new([0.0, 3.0], [0.0, 0.0], Some(4)),
        ];
        let plate = SpacePlate::build(&segs, BuildOptions::default());
        let room = plate.rooms().next().expect("a room");
        let o = plate.face_outline(room);
        assert_eq!(o.len(), 4, "still a quad");
        // Every corner is now square — the tilted wall was snapped onto the frame
        // so the room is a clean rectangle (possibly rotated a hair, since the one
        // tilted wall biases the detected frame in a 4-wall fixture).
        for i in 0..4 {
            let p = o[(i + 3) % 4];
            let c = o[i];
            let q = o[(i + 1) % 4];
            let e1 = [c[0] - p[0], c[1] - p[1]];
            let e2 = [q[0] - c[0], q[1] - c[1]];
            let cos = (e1[0] * e2[0] + e1[1] * e2[1])
                / (e1[0].hypot(e1[1]) * e2[0].hypot(e2[1]));
            assert!(cos.abs() < 1e-6, "corner {i} square after regularize, cos = {cos}");
        }
    }

    #[test]
    fn regularize_leaves_a_genuinely_angled_wall() {
        // A right triangle: the hypotenuse is 45° — far beyond the 3° tolerance,
        // so it must NOT be snapped to an axis (the area is preserved).
        let segs = vec![
            InputSegment::new([0.0, 0.0], [4.0, 0.0], Some(1)),
            InputSegment::new([4.0, 0.0], [0.0, 4.0], Some(2)), // 45° hypotenuse
            InputSegment::new([0.0, 4.0], [0.0, 0.0], Some(3)),
        ];
        let plate = SpacePlate::build(&segs, BuildOptions::default());
        let room = plate.rooms().next().expect("a room");
        // A 4×4 right triangle = 8 m²; snapping the hypotenuse to an axis would
        // square it off to 16 and is exactly what must NOT happen.
        assert!((plate.face_area(room) - 8.0).abs() < 1e-6, "angled wall preserved, area = {}", plate.face_area(room));
    }

    #[test]
    fn regularize_preserves_room_count_on_a_tilted_two_room_plate() {
        // Two rooms split by a shared wall, with the whole outer loop tilted a
        // hair. Regularization must keep BOTH rooms (topology-preserving) — the
        // failure mode of segment-side snapping was collapsing rooms together.
        let segs = vec![
            InputSegment::new([0.0, 0.0], [8.0, 0.03], Some(1)),
            InputSegment::new([8.0, 0.03], [8.03, 3.0], Some(2)),
            InputSegment::new([8.03, 3.0], [0.0, 3.0], Some(3)),
            InputSegment::new([0.0, 3.0], [0.0, 0.0], Some(4)),
            InputSegment::new([4.0, 0.015], [4.015, 3.0], Some(99)), // shared wall
        ];
        let plate = SpacePlate::build(&segs, BuildOptions::default());
        assert_eq!(plate.room_count(), 2, "both rooms survive regularization");
    }
