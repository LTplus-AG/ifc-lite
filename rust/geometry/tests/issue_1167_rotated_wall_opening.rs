...
//! Regression test for issue #1167 ("weird wall hole cutting").
//!
//! A wall rotated in plan had its window/door openings cut wrong in two ways:
//!
//! 1. **Over-cut.** Openings within ~18° of a world axis were routed onto the
//!    fast world-axis-aligned-AABB cut path (the old `is_axis_aligned_direction`
//!    tolerance of 0.95). The AABB of a *rotated* opening box is strictly larger
//!    than the box, so the cut removed wall outside the window — a hole bigger
//!    than the window and skewed to the world grid, not orthogonal to the wall.
//!
//! 2. **Fragmentation.** Cutting the tilted box in world space left rim slivers
//!    and hairline cracks (the exact kernel's coplanar re-triangulation is
//!    fragile on tilted planes), worst with the tessellated profiles real
//!    exporters emit.
//!
//! The fix tightens the axis tolerance to cos(1°) and, for a vertical wall
//! rotated in plan, cuts every opening in the wall's shared axis-aligned frame
//! (where the box cut is numerically clean) and rotates the result back. So a
//! rotated wall cuts like a straight one: exactly the window volume, watertight,
//! no slivers — independent of the rotation angle.
//!
//! Fixture: a 4.0 × 0.3 × 2.5 m wall rotated about Z with one rectangular window
//! (1.2 m × 1.5 m) punched through both faces (`opening` placed relative to the
//! wall, so it inherits the rotation). Correct removed volume = 1.2·1.5·0.3 =
//! 0.54 m³. The window profile can carry collinear edge midpoints (the
//! segmented profile that used to fragment the cut).
...
/// `(unpaired directed edges, needle triangles)`. Edges are paired on a 0.1 mm
/// quantised grid — the cut runs in a rotated frame and the rotate-back jitters
/// shared vertices in the 6th–7th f32 digit, so an exact-bit pairing would
/// report spurious openings; 0.1 mm is far below any real gap yet absorbs that
/// sub-micron jitter. `(0, 0)` ⇒ watertight and sliver-free.
...
fn rotated_wall_opening_is_not_overcut() {
    ...
    assert!(
        ...
        "opening removed {removed:.5} m³ (expected {WINDOW_VOLUME:.5}); well above \
         means the rotated opening was cut as its oversized world-axis bounding \
         box (issue #1167)"
    );
}
...
/// exactly the window volume. Pre-fix the tilted cut fragmented (open edges +
/// needles), worst at "unlucky" angles like 8° and 20°.
#[test]
fn rotated_opening_cuts_clean_at_every_angle() {
    for angle in [3.0, 8.0, 15.0, 20.0, 33.0, 45.0] {
        for tessellated in [false, true] {
            ...
            assert_eq!(
                open_edges, 0,
                "angle {angle}° tess={tessellated}: {open_edges} unpaired edges — \
                 the rotated cut fragmented (issue #1167)"
            );
            assert_eq!(
                needles, 0,
