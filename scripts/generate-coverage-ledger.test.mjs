...
test('real sources: generator succeeds and produces a non-empty, multi-section ledger', () => {
  ...
  assert.ok(ledger);
  assert.match(ledger, /## IFC2X3/);
  assert.match(ledger, /## IFC4X3/);
  assert.match(ledger, /\| IfcWall \|/);
});
...
test('vacuity guard: emptied processor_registry.rs TYPES array fails loudly, not silently', () => {
  ...
  assert.ok(src.includes('const TYPES'), 'test anchor drifted — real source no longer has `const TYPES`');
  ...
  assert.match(log, /found ZERO entries/);
...
test('mutation sensitivity: removing IfcSphere from the geometry TYPES table flips its row', () => {
  ...
  assert.match(beforeRow.split('|')[5].trim(), /✅/);
  assert.match(afterRow.split('|')[5].trim(), /❌/);
});
