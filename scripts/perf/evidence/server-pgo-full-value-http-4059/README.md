...
The published [v2 transport and witness tools](../server-pgo-darwin-2026-09-07/README.md) reproduce the cold upload and same-process hash-only replay workload. In a separate training-only copy of `run.py`, apply `training-harness-shutdown.patch`; the public file's license header shifts patch line numbers but leaves context unchanged. This closes the HTTP session, sends SIGINT and requires normal exit zero, instead of treating signal termination as sufficient. Do not apply it to timed control/use runs. Require exact raw and semantic cold/cache equality against validated stock-control captures for the same five fixtures before accepting their profiles.
