...
test('#5860 saved worker override is visible and resettable in Settings', async ({ page }, testInfo) => {
  ...
  await testInfo.attach('performance-overrides', { body: await page.screenshot(), contentType: 'image/png' });

  await dialog.getByRole('button', { name: 'Reset Geometry workers' }).click();
