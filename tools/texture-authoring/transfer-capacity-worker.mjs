...
// candidate runtime must return a complete applicable plan. Five fresh workers plus one
...
if (!rootArg || !inputPath || !outputPath) {
  throw new Error('Usage: node transfer-capacity-worker.mjs <candidate-worktree> <input.json> <output.json> [dependency-worktree]');
}
...
self.onmessage = async ({data}) => {
  ...
  try {
    ...
    try {
      const output = api.planMeshTransfer(content, json, pixels);
      const nativeMilliseconds = performance.now() - nativeStart;
      if (new TextDecoder().decode(output.subarray(0, 4)) !== 'IFPA') throw new Error('Missing IFPA output');
      const length = new DataView(output.buffer, output.byteOffset, output.byteLength).getUint32(4, true);
      if (length > output.length - 8) throw new Error('Invalid metadata length');
      const metadata = JSON.parse(new TextDecoder().decode(output.subarray(8, 8 + length)));
      if (!metadata.plan || !metadata.transfer.applicable || !metadata.assets.length) throw new Error('No applicable atlas plan');
      const sha256 = Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256', output)), n => n.toString(16).padStart(2, '0')).join('');
      self.postMessage({phase:'complete', nativeMilliseconds,
        workerMilliseconds:performance.now()-start, initialMemory, finalMemory:wasm.memory.buffer.byteLength,
        outputBytes:output.length, sha256, coverage:metadata.transfer.coverage, assets:metadata.assets,
        exclusions:metadata.plan.exclusions});
    } catch (error) {
      self.postMessage({
          ...
          return await new Promise((resolve, reject) => {
            ...
            worker.onmessage = ({ data }) => {
              ...
              if (data.phase === 'complete') {
                clearTimeout(timeout);
                if (data.refusal || !data.outputBytes) {
                  reject(new Error('Expected complete applicable full-target plan: ' + data.refusal));
                  return;
                }
